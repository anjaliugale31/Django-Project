...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        total_pages=len(pages) #total pages on site
        all_data=[]
        for tag in range(total_pages):
            if(count<page_no):
                ...
                if(Post.objects.filter(title=title_html)).exists():
                    continue
                else:
                    ...
                    if(text_img):
                        img_name=each_html(".cover img").attr("src").split("/")[-1] #getting name of img 
                        img_url="https://www.tutorialspoint.com"+text_img
                        with open('media/documents/'+img_name,'wb') as img_file:# 2nd way by creating new directory and displaying the image 
        ...
        #checking for total pages where scrap 
        if(count>0):
